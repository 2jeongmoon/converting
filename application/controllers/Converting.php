...
class Converting extends Base_Controller
{
	...
	function ajax_converting() {
		...
		if (empty($propertyInfo)) {
			$arrResult = array("code" => "error", "msg" => "Converting Error - platform.property 불러오기 실패", "log" => $arrReturnMsg);
		...
		if (!$seq_conference_room) {
			$arrResult = array("code" => "error", "msg" => "Converting Error - 매칭데이터 없음", "log" => $arrReturnMsg);
			echo json_encode($arrResult, true);
			exit;
		} else {
		...
		if (empty($arrBooking)) {
			...
			if ($insertBookingResult == "error") {
				$arrResult = array("code" => "error", "msg" => "Converting Error - Booking Insert 실패", "log" => $arrReturnMsg);
				echo json_encode($arrResult, true);
				exit;
			} else {
		...
		if (!$seq_account) {
			$arrResult = array("code" => "error", "msg" => "Converting Error - seq_account 불러오기 실패" , "log" => $arrReturnMsg);
			echo json_encode($arrResult, true);
			exit;
		} else {
		...
		if (!$s_type) {
			$arrResult = array("code" => "error", "msg" => "Converting Error - s_type 불러오기 실패" , "log" => $arrReturnMsg);
			echo json_encode($arrResult, true);
			exit;
		} else {
		...
		if ($s_type == "B2C") {
			...
			if (!$seq_b2c_cbs) {
				$arrResult = array("code" => "error", "msg" => "Converting Error - seq_b2c_cbs 불러오기 실패" , "log" => $arrReturnMsg);
				echo json_encode($arrResult, true);
				exit;
			} else {
		...
		if (empty($arrInvoiceSp)) {
			$arrResult = array("code" => "error", "msg" => "Converting Error - invoice_sp 불러오기 실패" , "log" => $arrReturnMsg);
			echo json_encode($arrResult, true);
			exit;
		} else {
		...
		if (!$seq_invoice) {
			$arrResult = array("code" => "error", "msg" => "Converting Error - seq_invoice 불러오기 실패", "log" => $arrReturnMsg);
			echo json_encode($arrResult, true);
			exit;
		} else {
		...
		if (empty($arrInvoiceDetailSp)) {
			$arrResult = array("code" => "error", "msg" => "Converting Error - invoice_detail_sp 불러오기 실패", "log" => $arrReturnMsg);
			echo json_encode($arrResult, true);
			exit;
		} else {
		...
		foreach ($arrInvoiceDetailSp as $k => $v) {
			...
			if ($insertInvoiceDetailResult == "error") {
				$arrResult = array("code" => "error", "msg" => "Converting Error - invoice_detail Insert 실패", "log" => $arrReturnMsg);
				echo json_encode($arrResult, true);
				exit;
			} else {
				$num = $k+1;
		...
		// all pass
		$arrResult = array("code" => "success", "msg" => "Converting Success", "log" => $arrReturnMsg);
		echo json_encode($arrResult, true);
		exit;
	}
